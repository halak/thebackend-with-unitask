...
namespace Halak.Editor
{
    public static class SourceCodeGenerator
    {
        ...
        {
            var backendAssemblyPath = Path.GetFullPath("Packages/com.halak.thebackend-with-unitask/Plugins/Backend.dll");
        ...
        {
            Generate(type, Path.GetFullPath($"Packages/com.halak.thebackend-with-unitask/Runtime/AutoGenerated/{type.Name}Extensions.cs"));
        }
        ...
        private static void Generate(TypeDefinition type, string outputPath)
        {
            ...
            var namespaces = new[] { type.Namespace, "System.Threading", "Cysharp.Threading.Tasks", }
            ...
            for (var i = 0; i < methodGroups.Length; i++)
            {
                foreach (var method in methodGroups[i])
                {
                    ...
                    var customAttributeSourceCodes = customAttributes.Select((customAttribute) =>
                    {
                        ...
                        return $"{tab}{tab}[{attributeName}({string.Join(", ", constructorArguments)})]";
                    }).ToArray();
                    ...
                    foreach (var line in customAttributeSourceCodes)
                    ...
                    foreach (var line in customAttributeSourceCodes)
                        writer.WriteLine(line);
                    writer.WriteLine($"{tab}{tab}public static UniTask<{returnType.GetReadableName()}> {method.Name}Async({string.Join(", ", parameters)}, CancellationToken cancellationToken)");
                    writer.WriteLine($"{tab}{tab}{tab}=> Asynchronizer.MakeWithCancellation({string.Join(", ", args)}, cancellationToken);");
                }
